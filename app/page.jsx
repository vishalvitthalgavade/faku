...
export default function Page(){
  ...
  const beginConfirm=(txn)=>{
    if(!txn?.name||!txn?.upiId){setToast('This QR does not contain a valid payee.');return;}
    setPending({...txn,amount:Number(txn.amount||0)});
    setScreen('confirm');
  };
  ...
  const completePayment=()=>{
    if(!pending||pending.amount<=0||processingTransaction)return;
    const entry={type:'sent',name:pending.name,upiId:pending.upiId,amount:Number(pending.amount),time:new Date().toISOString(),txnId:makeTransactionId()};
    ...
    setProcessingTransaction(entry);
    setScreen('processing');
    try {
      const audio = new Audio('/PhonePe-Ting-Ding.mp3');
      audio.volume = 0.9;
      audio.play().catch(()=>{});
    } catch {}
  };
  const finishProcessing=()=>{setProcessingTransaction(null);setScreen('transaction');};
  ...
  return <main id="app">
    ...
    {screen==='pin'&&<PinScreen txn={pending} onBack={()=>navigate('confirm')} onComplete={(pin)=>pin.length===4?completePayment():setToast('Enter all 4 digits.')} />}
    {screen==='processing'&&processingTransaction&&<PaymentProcessing transaction={processingTransaction} onDone={finishProcessing}/>} 
